...
__global__ void add(const double  *px, const double *py, double *pz, const int N) {
    // 寄存器(register)的可见范围是单个线程，生命周期是所属线程的生命周期，位置在 SM 上，且位于 GPU 片上，可读可写，
    ...
    // 局部内存（local memory），也叫本地内存，可见范围是单个线程，生命周期是所属线程的生命周期，位置在 GPU 设备，但位于 GPU 片外，可读可写，
...
int main(void) {
    ...
    // 全局内存（global memmory）的可见范围是整个网格，生命周期是整个应用程序（由主机分配和释放），位置在 GPU 设备，但位于片外，可读可写。
    // 全局内存的访问速度不快，但容量最大，基本就是显存的大小，
    // 类比： CUDA 的全局内存不是 C++ 中的全局变量所在的全局数据区，更类似 C++ 中的堆内存。
