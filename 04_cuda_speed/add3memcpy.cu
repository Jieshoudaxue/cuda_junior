...
int main(void) {
    ...
    for (int repeat = 0; repeat <= NUM_REPEATS; ++repeat) {
        ...
        // 将 CPU 到 GPU 之间数据的拷贝，加入度量范围
        // 针对双精度浮点数，我的机器的运行结果是：
        // GPU add() average elapsed time: 80.522713 ms, error: 0.116927 ms
        // 相比纯 CPU 的 26.976746 ms，还慢了两倍多，
        // 这说明如果计算量不大，GPU 的数据传输开销会抵消 GPU 的计算优势，这种情况下，使用纯 CPU 计算反而更快。
