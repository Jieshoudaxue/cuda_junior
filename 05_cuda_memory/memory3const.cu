...
// 常量内存（constant memory）的可见范围是整个网格，生命周期是整个应用程序（由主机分配和释放），位置在 GPU 设备，但位于片外，仅可读。
// 常量内存是一块特殊的全局内存，只读不可写，容量小（64KB），带有缓存，访问速度比全局内存高，
// 常量内存使用 __constant__ 修饰，属于设备，在所有主机和设备函数外定义，
// 类比：CUDA 的常量内存类似 C++ 的全局变量所在的全局数据区，但 C++ 的常量是编译器技巧，而这里的常量由内存特性保障
