# include <cstdio>
// 下面两个头文件是 CUDA 编程必用的头文件，nvcc 编译器会自动包含这两个头文件，用户不需要显式地包含这两个头文件。
// 因此，注销下面两行代码也不会影响程序的编译和运行。
#include <cuda.h>
...
int main(void) {
    ...
    // 线程先组织成线程块（同一个线程块中的线程，会被调度到同一个 SM 上运行），所有线程块再构成一个网格（grid），网格大小（grid size）就是线程块的个数，线程块大小（block size）就是线程块中线程的个数。
